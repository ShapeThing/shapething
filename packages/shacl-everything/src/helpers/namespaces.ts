...
/** ShapeThing namespace */
export const st: NamespaceBuilder<string> = namespace(
  "http://shapething.com/",
  {
    factory,
  },
...
/** ShapeThing convention: sh:datatype/a literal's datatype marking its lexical value as an
 * Iconify icon name (e.g. "mdi:home") - not a registered/dereferenceable datatype IRI, just a
 * sentinel st:IconifyEditor/st:IconifyViewer key off of (see their score.ttl). */
export const iconifyDatatype: NamedNode = factory.namedNode(
...
/** GeoSPARQL namespace - distinct from `geo` above (WGS84 Geo Positioning), despite both
 * vocabularies commonly being aliased "geo:" in the wild; kept separate here to avoid clashing
 * with the pre-existing `geo` export. */
export const geosparql: NamespaceBuilder<string> = namespace(
