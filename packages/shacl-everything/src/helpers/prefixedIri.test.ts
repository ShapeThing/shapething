...
test("prefixedIri - a sourcePrefixes entry reusing a built-in alias for a different namespace doesn't clobber it", () => {
  ...
  expect(prefixedIri(factory.namedNode("http://mycompany.example/thing"), sourcePrefixes)).toEqual(
    "ex:thing",
  );
  expect(prefixedIri(factory.namedNode("http://example.org/thing"), sourcePrefixes)).toEqual(
    "ex:thing",
  );
});
