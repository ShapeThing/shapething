...
test("toSparql - 4.2 Sequence Paths", async () => {
    ...
    expect(sparql).toBe(
        "<http://example.com/spouse>/<http://example.com/father>",
    );
});
...
test("toSparql - 4.3 Alternative Paths", async () => {
    ...
    expect(sparql).toBe(
        "<http://example.com/father>|<http://example.com/mother>",
    );
});
...
test("toSparql - alternative mixing a predicate and an inverse path", async () => {
    ...
    expect(sparql).toBe(
        "<http://example.com/father>|(^<http://example.com/child>)",
    );
});
...
test("toSparql - zero-or-more over a sequence", async () => {
    ...
    expect(sparql).toBe(
        "(<http://example.com/parent>/<http://example.com/sibling>)*",
    );
});
