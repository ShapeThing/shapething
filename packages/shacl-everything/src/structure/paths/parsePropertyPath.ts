...
function parsePathNode(pathNode: Term, shapesGraph: RdfStore): PropertyPath {
    ...
    const alternativePathQuads = shapesGraph.getQuads(
        pathNode,
        sh("alternativePath"),
    );
    if (alternativePathQuads.length > 0) {
        const items = getRdfList(
            alternativePathQuads[0].object,
            shapesGraph,
        ).map((item) => parsePathNode(item, shapesGraph));
    ...
    const zeroOrMorePathQuads = shapesGraph.getQuads(
        pathNode,
        sh("zeroOrMorePath"),
    );
    ...
    const oneOrMorePathQuads = shapesGraph.getQuads(
        pathNode,
        sh("oneOrMorePath"),
    );
    ...
    const zeroOrOnePathQuads = shapesGraph.getQuads(
        pathNode,
        sh("zeroOrOnePath"),
    );
    ...
    if (shapesGraph.getQuads(pathNode, rdf("first")).length > 0) {
        const items = getRdfList(pathNode, shapesGraph).map((item) =>
