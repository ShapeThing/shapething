    ...
    switch (path.type) {
        ...
        case "alternative": {
            ...
            if (
                branches.some(
                    ([instruction]) =>
                        instruction.start !== first.start ||
                        instruction.end !== first.end,
                )
            ) {
                throw new Error(
            ...
            if (
                branches.some(
                    ([instruction]) => instruction.quantifier !== first.quantifier,
                )
            ) {
                throw new Error(
            ...
            return [
                {
                    ...
                    predicates: branches.flatMap(
                        ([instruction]) => instruction.predicates,
                    ),
                },
            ];
        }
