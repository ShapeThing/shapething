...
export const keepHighestLiteral: ResolutionFunction<number | undefined> = (
    values,
) => {
...
export const keepLowestLiteral: ResolutionFunction<number | undefined> = (
    values,
) => {
    if (!values.length) return undefined;
    return literalOrder(values.reduce((
...
export const keepHighestInteger: ResolutionFunction<number | undefined> = (
    values,
) => {
    if (!values.length) return undefined;
...
export const keepLowestInteger: ResolutionFunction<number | undefined> = (
    values,
) => {
    if (!values.length) return undefined;
...
export const keepFirst: ResolutionFunction<Term | undefined> = (values) =>
    values[0];

export const keepAll: ResolutionFunction<Term[]> = (values) =>
    dedupeTerms(values);

export const keepAllListItems: ResolutionFunction<Term[]> = (
    values,
    element,
) => {
    return dedupeTerms(
...
export const keepListIntersection: ResolutionFunction<Term[]> = (
    values,
    element,
) => {
...
// for <input pattern>).
export const combinePatterns: ResolutionFunction<RegExp | undefined> = (
    values,
) => {
...
export const resolveBooleans: ResolutionFunction<boolean | undefined> = (
    values,
) => {
    if (!values.length) return undefined;
...
export const enforceSame: ResolutionFunction<Term | undefined> = (
    values,
    _element,
    predicate,
) => {
...
export const keepMostSpecificClasses: ResolutionFunction<Term[]> = (
    values,
    element,
) => {
    ...
    for (const classEntry of classes) {
        ...
        while (frontier.length > 0) {
            ...
            for (const node of frontier) {
                for (
                    const quad of element.shapesGraph.getQuads(
                        node,
                        rdfs("subClassOf"),
                    )
                ) {
...
export const keepMostSevere: ResolutionFunction<Term | undefined> = (
    values,
) => {
    if (!values.length) return undefined;
...
export const nodeKindIntersection: ResolutionFunction<Term[]> = (
    values,
    element,
) => {
    if (!values.length) return [];
