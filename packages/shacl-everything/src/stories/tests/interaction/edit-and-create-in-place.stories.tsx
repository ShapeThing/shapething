...
export default {
  title: "Tests/Interaction/Edit and create in place",
...
export const editingTheSelectedAutoCompleteReferenceInPlace: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.clear(nameInput);
    await userEvent.type(nameInput, "AcmeCorpSponsor");
    await userEvent.tab();
    await expect(dialogScope.findByDisplayValue("AcmeCorpSponsor")).resolves.toBeVisible();

    await userEvent.click(dialogScope.getByRole("button", { name: /update/i }));
    await waitFor(() => expect(body.queryByRole("dialog")).toBeNull());

    await expect(within(trigger).findByText("AcmeCorpSponsor")).resolves.toBeVisible();
  },
};
