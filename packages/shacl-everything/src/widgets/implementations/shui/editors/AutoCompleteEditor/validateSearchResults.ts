...
// WHERE" or "?value" in "SELECT ?value {" - mirrors query.ts's toResolvedTerms convention ("Use
...
/**
 * Rewrites `query` (an sh:in [ sh:select ] body) to pre-bind its first projected `variable` to
 * exactly `values`, via an injected VALUES clause - so running the rewritten query returns only
 * the subset of `values` the query's own pattern actually accepts, without ever pulling its full,
 * independently-sized baseline result set. Inserted right inside the first SERVICE block's braces
 * when there is one (so it's evaluated as part of that single remote request, not bind-joined
 * against it locally - see the module docstring), otherwise right inside the outermost `{`
 * (covering both `WHERE {` and the `WHERE`-less `SELECT ?value { ... }` form).
 *
 * Text-level insertion, not a full SPARQL parse - consistent with extractServiceEndpoint/
 * substituteSearchParameters elsewhere in this codebase, which take the same lightweight-regex
