...
// Exported so a caller-supplied `interfaceLocales` loader can reuse the same
// fetch-with-error-checking logic as the built-in loaders below, instead of duplicating it.
...
// The shape of the `interfaceLocales` prop on ShaclManager: a custom loader for a tag adds a
// brand new locale or overrides a built-in one with the caller's own translation, and `null`
// removes a built-in entirely - e.g. to ship with just one interface language.
export type LocaleLoaderOverrides = Record<string, LocaleLoader | null>;

// Locales shipped with this package out of the box. Consumers can add further locales, override
// these, or remove one - via the `interfaceLocales` prop on ShaclManager (see loadBundles.ts).
