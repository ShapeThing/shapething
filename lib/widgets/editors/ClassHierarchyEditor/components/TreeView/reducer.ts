...
export const treeReducer = (state: ITreeViewState, action: TreeViewAction): ITreeViewState => {
  switch (action.type) {
    ...
    case treeTypes.expandMany: {
      const expandedIds = new Set<NodeId>([...state.expandedIds, ...action.ids])
      return {
        ...state,
        expandedIds,
        lastAction: action.type,
    ...
    case treeTypes.select: {
      ...
      const isFocused = tabbableId === action.lastInteractedWith || action.NotUserAction !== true
    ...
    case treeTypes.controlledSelectMany: {
      ...
      if (action.multiSelect) {
        ...
        if (action.ids.length > 1) {
          console.warn('Tree in singleSelect mode, only the first item from selectedIds will be selected.')
        }
