...
import { baseClassNames, CLICK_ACTIONS, clickActions, NODE_ACTIONS, treeTypes } from './constants'
...
const useTree = ({
  ...
  useEffect(() => {
    if (onSelect != null && onSelect !== noop) {
      for (const toggledId of toggledIds) {
        const isBranch = isBranchNode(data, toggledId) || !!getTreeNode(data, tabbableId)?.isBranch
        onSelect({
    ...
    if (prevData !== data) {
      ...
      if (treeParentNode.children.length) {
        dispatch({
          ...
          tabbableId: !data.find(node => node.id === state.tabbableId) ? treeParentNode.children[0] : state.tabbableId,
          lastInteractedWith: !data.find(node => node.id === state.lastInteractedWith)
            ? null
            : state.lastInteractedWith,
          lastManuallyToggled: !data.find(node => node.id === state.lastManuallyToggled)
            ? null
  ...
  const toggledControlledIds = symmetricDifference(new Set(controlledSelectedIds), selectedIds)

  useEffect(() => {
    if (!!controlledSelectedIds) {
      if (toggledControlledIds.size) {
        dispatch({
      ...
      for (const id of controlledSelectedIds) {
        if (propagateSelect && !disabledIds.has(id)) {
          dispatch({
            type: treeTypes.changeSelectMany,
            ids: propagatedIds(data, [id], disabledIds),
            select: true,
      ...
      if (controlledSelectedIds) {
        idsToUpdate.forEach(id => {
          if (isBranchNode(data, id)) {
            ...
            const isPropagatedId = descendantIds.every(childId => selectedIds.has(childId))
            if (isPropagatedId) {
              every.add(id)
            }
          }
      ...
      for (const id of every) {
        if (!selectedIds.has(id)) {
          dispatch({
            ...
            id,
            multiSelect: multiSelect ? multiSelect : isBranchNotSelectedAndHasOnlySelectedChild(data, id, selectedIds),
            keepFocus: true,
            NotUserAction: true,
            lastInteractedWith
          })
  ...
  useEffect(() => {
    if (lastInteractedWith == null) return
      ...
      const isTreeActive =
        treeRef?.current == null || (document.activeElement && treeRef.current.contains(document.activeElement))
...
const TreeView = React.forwardRef(function TreeView<M extends IFlatMetadata = IFlatMetadata>(
  {
    data,
    selectedIds,
    nodeRenderer,
  ...
  (event: React.KeyboardEvent) => {
    ...
    if (event.ctrlKey) {
      if (event.key === 'a' && clickAction !== clickActions.focus) {
        ...
        const ids = dataWithoutRoot.map(x => x.id).filter(id => !disabledIds.has(id))
        dispatch({
          type: treeTypes.changeSelectMany,
          multiSelect,
        ...
        const newId = event.key === 'Home' ? getTreeParent(data).children[0] : getLastAccessible(data, id, expandedIds)
    ...
    switch (event.key) {
      ...
      case 'Spacebar':
        ...
        dispatch({
          type: togglableSelect ? getOnSelectTreeAction(data, id, selectedIds, disabledIds) : treeTypes.select,
          id: id,
          multiSelect,
          lastInteractedWith: id,
          lastManuallyToggled: id
        })
        if (propagateSelect && !disabledIds.has(element.id)) {
          dispatch({
            type: treeTypes.changeSelectMany,
            ids: propagatedIds(data, [id], disabledIds),
      ...
      default:
        if (event.key.length === 1) {
          ...
          while (currentId !== id) {
            ...
            if (getTreeNode(data, currentId).name[0].toLowerCase() === event.key.toLowerCase()) {
              dispatch({
                type: treeTypes.focus,
                id: currentId,
                lastInteractedWith: id
