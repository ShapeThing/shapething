...
export interface INodeGroupProps<M extends IFlatMetadata = IFlatMetadata> extends Omit<
  INodeProps<M>,
  'setsize' | 'posinset'
> {
...
export const Node = <M extends IFlatMetadata = IFlatMetadata>(props: INodeProps<M>) => {
  ...
  const handleExpand: EventCallback = event => {
    ...
    if (expandedIds.has(element.id) && propagateCollapse) {
      const ids: NodeId[] = [element.id, ...getDescendants(data, element.id, new Set<NodeId>())]
      dispatch({
  ...
  const handleSelect: EventCallback = event => {
    if (event.shiftKey) {
      ...
      //Select
      dispatch({
        type: togglableSelect ? getOnSelectTreeAction(data, element.id, selectedIds, disabledIds) : treeTypes.select,
        id: element.id,
        multiSelect,
        lastInteractedWith: element.id,
        lastManuallyToggled: element.id
  ...
  const getLeafProps = (args: { onClick?: EventCallback } = {}) => {
    ...
    return {
      ...
      onClick: onClick == null ? composeHandlers(handleSelect, handleFocus) : composeHandlers(onClick, handleFocus),
      ref: (x: INodeRef) => {
