...
export const usePrevious = (x: Set<NodeId>) => {
  const ref = useRef<Set<NodeId>>(null)
  useEffect(() => {
    ref.current = x
  }, [x])
  return ref.current
...
export const getBranchNodesToExpand = (data: INode[], id: NodeId): NodeId[] => {
  ...
  const isNodeExpandable = parentId && (isBranchNode(data, parentId) || getTreeNode(data, parentId).isBranch)
...
export const getAncestors = (data: INode[], childId: NodeId, disabledIds: Set<NodeId>) => {
...
export const getDescendants = (data: INode[], id: NodeId, disabledIds: Set<NodeId>) => {
...
export const getLastAccessible = (data: INode[], id: NodeId, expandedIds: Set<NodeId>) => {
  let node = getTreeNode(data, id)
  const isRoot = getTreeParent(data).id === id
  if (isRoot) {
    node = getTreeNode(data, getTreeNode(data, id).children[getTreeNode(data, id).children.length - 1])
  }
...
export const getPreviousAccessible = (data: INode[], id: NodeId, expandedIds: Set<NodeId>) => {
...
export const getNextAccessible = (data: INode[], id: NodeId, expandedIds: Set<NodeId>) => {
...
export const propagateSelectChange = (
  ...
  for (const id of ids) {
    ...
    while (true) {
      ...
      const enabledChildren = getTreeNode(data, parent).children.filter(x => !disabledIds.has(x))
      ...
      if (!some) {
        const selectedAncestorId = getAncestors(data, currentId, disabledIds).find(id => {
          return selectedIds.has(id)
        })
        if (!multiSelect && selectedAncestorId) {
          const descendants = getDescendants(data, selectedAncestorId, disabledIds)
...
export const flattenTree = <M extends IFlatMetadata = IFlatMetadata>(tree: ITreeNode<M>): INode<M>[] => {
  let internalCount = 0
  ...
  const flattenTreeHelper = (tree: ITreeNode<M>, parent: NodeId | null) => {
    ...
    if (flattenedTree.find(x => x.id === node.id)) {
      throw Error(`Multiple TreeView nodes have the same ID (${node.id}). IDs must be unique.`)
    }
...
export const propagatedIds = (data: INode[], ids: NodeId[], disabledIds: Set<NodeId>) =>
...
export const onComponentBlur = (event: React.FocusEvent, treeNode: HTMLUListElement | null, callback: () => void) => {
  ...
  } else if (!treeNode.contains(event.nativeEvent.relatedTarget as Node)) {
    callback()
  }
}
...
export const isBranchSelectedAndHasSelectedDescendants = (
  ...
  return (
    ...
    getDescendants(data, elementId, new Set<number>()).some(item => selectedIds.has(item))
  )
}
...
export const isBranchSelectedAndHasOnlySelectedChild = (data: INode[], elementId: NodeId, selectedIds: Set<NodeId>) => {
  const nodeChildren = getChildren(data, elementId)
  return (
    isBranchNode(data, elementId) &&
    selectedIds.has(elementId) &&
...
export const getOnSelectTreeAction = (
  ...
  const isSelectedAndHasSelectedDescendants = isBranchSelectedAndHasSelectedDescendants(data, elementId, selectedIds)

  const isSelectedAndHasOnlySelectedChild = isBranchSelectedAndHasOnlySelectedChild(data, elementId, selectedIds)
...
export const getTreeParent = <M extends IFlatMetadata = IFlatMetadata>(data: INode<M>[]): INode<M> => {
  const parentNode: INode<M> | undefined = data.find(node => node.parent === null)
